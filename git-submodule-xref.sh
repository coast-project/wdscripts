...
# Find commits in referencing repository which match the submodule path and create matching tags
# in both repositories before rewriting either history to be able to find the corresponding
# entry again
createReferenceTags()
{
    tagnum=0
...
# filters referencing repository and replace old with new submodule commit hashes
replaceHashes()
{
    tagnum=0
...
USAGE="[--xrefprefix xrefTagPrefix] [--tagnamesep tagNameSeparator] [--outputsep outputSeparator] [--refrepo referencingRepositoryName] [--dorevs revisionToWorkOn] [--dry-run] [--store] submodule_dir"
...
store= filter referencing repository and replace old with new submodule commit hashes
"
