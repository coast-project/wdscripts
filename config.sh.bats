...
# ├── config
# └── scripts
@test "PROJECTDIR setting from samedir" {
...
# ├── config
# └── scripts
@test "PROJECTDIR setting from subdir" {
...
# └── scripts
@test "PROJECTDIR setting from linked subdir" {
...
# ├── lnscripts -> /tmp/tmp.KQ6AdNvese/scripts
# └── scripts
@test "PROJECTDIR setting from linked subdir absolute" {
  ln -s ${tdir_scripts} ${tdir_lnscripts}
  tree -d $tdir
...
@test "PROJECTDIR setting from within projectpath with linked subdirs" {
  mkdir -p ${tdir}/logs
...
# ├── logs -> /tmp/tmp.1/logs
# └── scripts -> /tmp/tmp.1/scripts
@test "PROJECTDIR setting from within config subdir of projectpath with linked subdirs" {
  mkdir -p ${tdir}/logs
  for d in config logs scripts; do ln -s ${tdir}/$d ${tdir2}/$d; done
  tree -d $tdir $tdir2
...
# ├── logs -> /tmp/tmp.1/logs
# └── scripts -> /tmp/tmp.1/scripts
@test "PROJECTDIR setting from within logs subdir of projectpath with linked subdirs" {
  mkdir -p ${tdir}/logs
  for d in config logs scripts; do ln -s ${tdir}/$d ${tdir2}/$d; done
  tree -d $tdir $tdir2
...
# ├── logs -> /tmp/tmp.2/logs
# └── scripts -> /tmp/tmp.1/scripts
@test "PROJECTDIR setting from within logs/rotate subdir of projectpath with linked subdirs" {
