...
# generated using $> sed -n 's/^\([a-zA-Z][^(]*\)(.*$/unset -f \1/p' startwda.sh | grep -v "\$$"
unset -f showhelp
unset -f nologExit
unset -f myExit
unset -f exitproc
...
showhelp()
{
	...
	[ -n "${1}" ] && echo "${1}";
	echo ""
	echo "usage: $script_name [options] -- [app-params]..."
...
echo ""
echo "------------------------------------------------------------------------"
echo "$script_name - script to start application [${SERVERNAME}] on [${HOSTNAME}]"
echo ""
